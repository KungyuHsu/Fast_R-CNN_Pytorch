...
def get_list(src_xml,src_jpg):
    ...
    return positive[:16],negative[:48],bbs
...
def pick_train():
    ...
    for i, sample in enumerate(samples):
        ...
        positive_list, negative_list, bbs = get_list(src_xml, src_jpg)
        ...
        dst_annotation_bbs_path = os.path.join(dst_annotation, sample + '_-1' + '.csv')
        ...
        print('{}parse {}.png in {:.0f}m {:.0f}s process: [{}/{}] {:.2f}%'.format(name, sample, time_elapsed // 60,
                                                                                  time_elapsed % 60, i, tlt,
                                                                                  (i / tlt) * 100))
    # 保存提取出来的编号列表
